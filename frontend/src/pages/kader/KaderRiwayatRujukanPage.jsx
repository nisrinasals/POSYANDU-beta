...
export default function KaderRiwayatRujukanPage({ globalSasaranList = [], globalPemeriksaanData = {}, onNavigate }) {
  ...
  const filteredReferrals = useMemo(() => {
    return allReferrals.filter((item) => {
      ...
      const matchKategori = kategoriFilter === "Semua Kategori" || item.kategori === kategoriFilter || item.kategoriKey.includes(kategoriFilter.toLowerCase().replace(/\s+/g, "-"));
        ...
        <div className="row g-3 mb-4 align-items-center">
          <div className="col-12 col-md-5">
            <div className="position-relative">
              ...
              <input type="text" className="form-control form-control-custom ps-5 bg-light" placeholder="Cari Nama Lengkap, NIK, Kader, atau Indikasi..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
          ...
          <table className="table table-custom align-middle">
            <thead>
              <tr className="text-muted small text-uppercase fw-bold border-bottom">
                ...
                <th className="py-3 text-center" style={{ minWidth: "140px", whiteSpace: "nowrap" }}>
                  TANGGAL DIRUJUK
                </th>
                <th className="py-3" style={{ minWidth: "240px" }}>
                  INDIKASI / MASALAH RUJUKAN
                </th>
                <th className="pe-4 py-3 text-center" style={{ width: "90px" }}>
                  AKSI
                </th>
              </tr>
            </thead>
        ...
        <div className="modal show d-block" tabIndex="-1" style={{ backgroundColor: "rgba(0,0,0,0.5)", zIndex: 1060 }}>
          <div className="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
            <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
              {/* Modal Header */}
              <div className="modal-header text-white p-3 px-4 d-flex align-items-center justify-content-between" style={{ backgroundColor: "#2b2e4a" }}>
                <div>
                ...
                <button type="button" className="btn-close btn-close-white" onClick={handleCloseDetail} aria-label="Tutup"></button>
              </div>
                  ...
                  <div className="row g-3">
                    ...
                    <div className="col-12 col-md-6">
                      <div className="p-3 bg-light rounded-3 h-100">
                        ...
                        <div className="fw-bold text-dark fs-6">{selectedReferral.kategori}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="row g-3">
                    <div className="col-12 col-md-6">
                      <div className="p-3 bg-light rounded-3 h-100">
                        ...
                        <div className="fw-bold text-dark fs-6">{selectedReferral.kaderPerujuk || ""}</div>
                      </div>
                    </div>
                    <div className="col-12 col-md-6">
                      <div className="p-3 bg-light rounded-3 h-100">
              ...
              <div className="modal-footer bg-white p-3 px-4 d-flex justify-content-between align-items-center">
                <span className="badge bg-danger-subtle text-danger border border-danger-subtle px-2.5 py-1.5 rounded-pill">Status: Memerlukan Penanganan Puskesmas / Pustu</span>
                <button type="button" className="btn btn-outline-secondary btn-sm px-4 rounded-3" onClick={handleCloseDetail}>
                  Tutup
                </button>
              </div>
            </div>
