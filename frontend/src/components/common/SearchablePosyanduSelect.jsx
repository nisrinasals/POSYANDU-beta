...
export default function SearchablePosyanduSelect({ value = "", onChange, placeholder = "Ketik untuk mencari nama Posyandu / Kelurahan...", disabled = false, required = false, name = "posyandu", id, className = "", onSelectPosyandu }) {
  ...
  // Load the public Posyandu directory from the backend. No static fallback is used.
  useEffect(() => {
    let cancelled = false;
    const loadPosyandu = async () => {
      setLoading(true);
      try {
        const res = await posyanduService.getPublicPosyanduList();
  ...
  return (
    <div className={`position-relative ${className}`} ref={containerRef} style={{ width: "100%" }}>
      ...
      >
        ...
        />
        {value && !disabled && (
          <button type="button" className="btn btn-link text-muted p-1 me-1 text-decoration-none border-0" onClick={handleClear} title="Hapus pilihan" tabIndex={-1}>
            <X size={14} />
          </button>
        )}
        <button
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="d-flex align-items-center justify-content-between px-3 py-1.5 bg-light border-bottom text-muted" style={{ fontSize: "0.725rem" }}>
            <span>{searchTerm ? `Hasil pencarian "${searchTerm}" (${filteredList.length})` : `Menampilkan ${filteredList.length} Posyandu`}</span>
            <span>Ketik untuk menyaring</span>
          </div>

          {/* Results List */}
          <div ref={listRef} className="overflow-y-auto" style={{ maxHeight: "235px" }}>
            {loading ? (
              ...
              filteredList.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className={`d-flex align-items-center justify-content-between px-3 py-2 cursor-pointer border-bottom border-light ${isSelected ? "bg-primary-subtle text-primary" : isHighlighted ? "bg-light text-dark" : "text-dark"}`}
                    style={{
                      cursor: "pointer",
                    ...
                    <div className="overflow-hidden py-0.5">
                      ...
                      <div className="text-muted d-flex align-items-center gap-1 text-truncate" style={{ fontSize: "0.725rem", fontWeight: 400 }}>
                        ...
                        <span className="text-truncate">
                          Kel. {item.kelurahan} • Pkm. {item.puskesmas}
                        </span>
                      </div>
                    </div>

                    {isSelected && <Check size={15} className="text-primary flex-shrink-0 ms-2" />}
                  </div>
                );
              })
            ) : (
