...
import { authService, wargaService, sesiService, pemeriksaanService, userService, posyanduService } from "./services";
...
export default function App() {
  ...
  const fetchBackendData = async () => {
    ...
    let backendPemMap = {};
    try {
      const resPem = await pemeriksaanService.getPemeriksaanList({ page: 1, limit: 1000 });
    ...
    try {
      const resSesi = await sesiService.getSesiList({ page: 1, limit: 1000 });
  ...
  const handleLoginSuccess = (loginData) => {
    const backendRole = loginData.roleType || loginData.role || "kader";
    const roleMap = {
      kader: ["kader", "Kader"],
    ...
    const [roleType, roleTitle] = roleMap[backendRole] || ["kader", "Kader"];
  ...
  if (isDinkes) {
    return (
      <AppLayout user={user} activeMenu={activeMenu} activeSubmenu={activeSubmenu} onNavigate={handleNavigate} onLogout={handleLogout} onToggleRole={handleToggleRole}>
