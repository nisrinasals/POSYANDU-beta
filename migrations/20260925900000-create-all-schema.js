...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    await queryInterface.addConstraint("users", {
      ...
      where: {
        status: ["pending_approval", "rejected", "active", "inactive"],
      },
    });
    ...
    await queryInterface.sequelize.query(`
      ALTER TABLE pemeriksaan
        ADD CONSTRAINT chk_pemeriksaan_bb
          CHECK (bb_kg IS NULL OR bb_kg > 0),
        ADD CONSTRAINT chk_pemeriksaan_tb
          CHECK (tb_cm IS NULL OR tb_cm > 0),
        ADD CONSTRAINT chk_pemeriksaan_sistole
          CHECK (td_sistole IS NULL OR td_sistole > 0),
        ADD CONSTRAINT chk_pemeriksaan_diastole
    ...
    await queryInterface.sequelize.query(`
      ...
      ON pemeriksaan USING GIN (detail_skrining);
    `);
    ...
    await queryInterface.addIndex("rujukan", ["warga_id"], {
